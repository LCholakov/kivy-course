...
class MainWidget(RelativeLayout):
    ...
    V_NB_LINES = 16
    V_LINES_SPACING = .55   # percentage of screen width .35 looks good
    vertical_lines = []
    ...
    def init_ship_outline(self):
        with self.canvas:
            Color(1, 1, 1)
            self.ship_outline = Line()

    def update_ship(self):
        center_x = self.width / 2
        base_y = self.height * self.SHIP_BASE
        ...
        self.ship.points = [x1, y1,
                            x2, y2,
                            x3, y3]


    def update_ship_outline(self):
        x1, y1 = self.transform(*self.ship_coordinates[0])
        x2, y2 = self.transform(*self.ship_coordinates[1])
        x3, y3 = self.transform(*self.ship_coordinates[2])

        self.ship_outline.width = 1
        self.ship_outline.points = [x1, y1,
                                    x2, y2,
                                    x3, y3,
                                    x1, y1]
    ...
    def get_tile_coordinates(self, ti_x, ti_y):
        ...
        y = self.get_line_y_from_index(ti_y - self.current_y_loop)
        return x, y
    ...
    def update(self, dt):
        ...
        self.update_tiles()
        self.update_ship()
        self.update_ship_outline()

        spacing_y = self.H_LINES_SPACING * self.height

        if self.state_game_has_started and not self.state_game_over:

            speed_y = self.current_speed_y * self.height / 100 + self.current_y_loop * 0.01
            self.current_offset_y += speed_y * time_factor

            while self.current_offset_y >= spacing_y:
                self.current_offset_y -= spacing_y - speed_y
                self.current_y_loop += 1
                self.score_text = "SCORE: " + str(self.current_y_loop)
                self.generate_tile_coordinates()

            speed_x = self.current_speed_x * self.width / 100 + self.current_y_loop * 0.01
        ...
        if not self.check_ship_collision() and not self.state_game_over:
            ...
            Clock.schedule_once(self.play_game_over_voice, .5)
            print("GAME OVER")
